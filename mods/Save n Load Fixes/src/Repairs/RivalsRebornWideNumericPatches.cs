...
namespace SaveNLoadFixes.Repairs
{
    ...
    /// <summary>
    /// Stable vanilla bootstrap for the optional Rivals Reborn profile. The external RR
    /// methods are patched dynamically by RivalsRebornWideNumericInterop only after an RR
    /// assembly is actually present, so disabled/absent RR can never invalidate PatchAll.
    /// </summary>
    [HarmonyPatch]
    internal static class RivalsReborn_WideNumericBootstrap_Patch
    {
        private static MethodBase TargetMethod()
        {
            RivalsRebornWideNumericInterop.SafeEnsureInitialized();
            MethodInfo method = AccessTools.Method(
                typeof(Mods), nameof(Mods.LoadMods), Type.EmptyTypes);
            if (method == null)
