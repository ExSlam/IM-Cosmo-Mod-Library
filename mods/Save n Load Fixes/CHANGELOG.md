## Unreleased - runtime qualification fixes

- Fixed the v0.52 header-only repair-envelope failure. SNLF no longer asks Unity's type-layout serializer to emit its nested repair DTO graph; a bounded public-field JSON writer now emits the complete finite envelope, rejects cycles/non-finite numbers, and must pass raw `records`/`relationship_dynamics` presence checks plus a full Unity deserialize/value comparison before a write can enter the queue.
