...
The release version is 3.3.0, while the compact sidecar `FormatVersion` remains `3`. Version 3.3 adds a sibling implementation-detail journal (`IMDataCore.LightweightJournal`, current format 2; legacy format 1 remains readable) but does not add a required field to the v3 base document, so older v3 documents remain valid.
