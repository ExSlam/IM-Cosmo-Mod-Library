...
IMDC 3.2 still writes `IMDataCore.LightweightSidecar` format version 3. The new `IdempotencyKey` event member is optional and appears only on custom events written through `TryAppendCustomEventOnce`.
...
IMDC 3.2 reads format versions 1 and 2, validates them, rebuilds runtime state, and continues normally. The next successful sidecar persistence writes format version 3.
...
An existing sidecar must contain an exact checkpoint for the vanilla save being loaded. IMDC 3.2 no longer activates unmatched sidecars by an in-game-date cutoff.
...
If the primary sidecar is unreadable or invalid, IMDC 3.2 validates `<sidecar>.imdc.bak`. A valid backup may become the recovery source for the session, but it still must contain an exact checkpoint for the loaded vanilla save.
...
IMDC 3.2 does not perform runtime migration from the historical database-backed persistence system.
...
IMDC 3.2 refuses to overwrite a sidecar whose format version is newer than it understands. This makes accidental downgrade safer.
